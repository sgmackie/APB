...
//Prototypes
PAN_POSITION simple_pan_stereo(double PanPosition);
...
int main(int argc, char *argv[])
{
    ...
    FramesRead_Total = 0;
    PanPosition_Current = simple_pan_stereo(PanPosition);
    ...
    while((FramesRead = psf_sndReadFloatFrames(InputFile, FramesInput, BufferSize)) > 0 )
    {
        ...
        double StereoPan_Position;

        for(Input_i = 0, Output_i = 0; Input_i < FramesRead; Input_i++)
        {
            StereoPan_Position = breakpoint_value_at_breakpoint_time(ReadPoints, BreakpointFile_Size, SampleTime);
            PanPosition_Current = simple_pan_stereo(StereoPan_Position);
...
PAN_POSITION simple_pan_stereo(double PanPosition)
{
    ...
    PanPosition *= 0.5;
    StereoPosition.PanLeft = PanPosition - 0.5;
    StereoPosition.PanRight = PanPosition + 0.5;
